...
public class PassordUtil {
	...
	/*
	 * Man kunne godt gjort denne klassen mer fleksibel ved � bytte ut konstanter
	 * med konstrukt�rparametre. P� den annen side: MAN M� BRUKE SAMME OPPSETT
	 * OVERALT I EN APPLIKASJON.
	 */
	 ...
	 * 
	 * Det er ikke noen spesiell grunn til at salt og digest b�r sl�s sammen til �n
	 * streng. De kunne godt v�rt returnert som to verdier (som et par). Det er
	 * gjort slik fordi det gjorde API-et enkelt � bruke, en verdi inn, en verdi ut.
	 * 
	...
	/**
	 * Denne metoden sjekker om angitt passord matcher kryptert passord (dvs. salt +
	 * digest) som tidligere er generert med {@link #krypterPassord(String)}.
	 * 
	 * @param passord  Passord som skal sjekkes
	 * @param kryptert (salt + digest) tidligere generert med
	 *                 {@link #krypterPassord(String)}
	...
	private static String krypterMedSalt(byte[] salt, String passord) {
		...
		try {
			 ...
			 * passord.getBytes(PASSORD_TEGNSETT) eller ugyldig angivelse av hash-algoritme
			 * i MessageDigest.getInstance(HASH_ALGORITME). Unntak vil normalt ikke
			 * plutselig oppst� under kj�ring, og det er derfor kanskje ikke n�dvendig �
			 * h�ndtere dem her. Eller...?
			 * 
			 * Hvis det skjer en feil, f.eks. at SHA-2 ikke er implementert i aktuell JVM,
			 * vil resultatet av � sluke unntaket bli at vi returnerer en tom streng. Ved
			 * senere passord-matching vil alle passord matche de som er generert med denne
			 * feilen.
			 * 
